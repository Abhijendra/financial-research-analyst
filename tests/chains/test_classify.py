...
def test_build_classify_chain():
    fake_model = FakeChatModel(DocumentClassification(document_type="earnings_report",confidence=0.6,reason="x"))
    chain = build_classify_chain(model=fake_model)
    result: DocumentClassification = chain.invoke({"document_text":"Q3 earning report blah blah..."})

    assert result.document_type == "earnings_report"
    assert result.confidence == 0.6
    
def test_downgrade_if_unsure():
    fmodel = FakeChatModel(DocumentClassification(document_type="earnings_report", confidence=0.3, reason="y"))
