...
from analyst.extraction.structured import (
    build_financials_extractor,
    build_risks_extractor,
    build_sentiment_extractor,
)
...
def build_news_chain(model: Runnable | None = None) -> Runnable:
    ...
    model = model or get_resilient_model()
    return RunnableParallel(
        risks=build_risks_extractor(model), sentiment=build_sentiment_extractor(model)
    )
...
def build_default_chain(model: Runnable | None = None) -> Runnable:
    ...
    model = model or get_resilient_model()
    return RunnableParallel(sentiment=build_sentiment_extractor(model))
